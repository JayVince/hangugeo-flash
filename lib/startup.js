...
async function runStartup() {
  ...
  console.log('🔍 Paramètres DB lus par Node.js :', {
    DB_SOCKET:   process.env.DB_SOCKET   || '(non défini)',
    DB_HOST:     process.env.DB_HOST     || '(non défini)',
    DB_USER:     process.env.DB_USER     || '(non défini)',
    DB_PASSWORD: pwd ? `${'*'.repeat(Math.max(0, pwd.length - 2))}${pwd.slice(-2)} (${pwd.length} car.)` : '(non défini)',
